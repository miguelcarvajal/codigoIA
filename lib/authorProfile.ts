...
export function parseAndValidateAuthorUrl(input: string): AuthorContext {
...
export async function collectAuthorPreviews(context: AuthorContext): Promise<ArticlePreview[]> {
...
export async function enrichArticles(items: ArticlePreview[], fallbackAuthor: string, context?: AuthorContext): Promise<ArticlePreview[]> {
...
const STOPWORDS_ES = new Set([
  "de","la","que","el","en","y","a","los","del","se","las","por","un","para","con","no","una","su","al","lo","como","más","pero","sus","le","ya","o","fue","ha","sí","porque","esta","son","entre","cuando","muy","sin","sobre","también","me","hasta","hay","donde","quien","desde","todo","nos","durante","todos","uno","les","ni","contra","otros","ese","eso","ante","ellos","e","esto","mí","antes","algunos","qué","unos","yo","otro","otras","otra","él","tanto","esa","estos","mucho","quienes","nada","muchos","cual","poco","ella","estar","estas","algunas","algo","nosotros","mi","mis","tú","te","ti","tu","tus","ellas","nosotras","vosostros","vosostras","os","mío","mía","míos","mías","tuyo","tuya","tuyos","tuyas","suyo","suya","suyos","suyas","nuestro","nuestra","nuestros","nuestras","vuestro","vuestra","vuestros","vuestras","esos","esas","estoy","está","estamos","están","esté","estén","ser","era","eran","soy","somos","sea","sean","siendo","sido"
]);
...
export function buildAuthorProfile(items: ArticlePreview[]): AuthorProfile {
  ...
  items.forEach((item) => {
    const text = `${item.title} ${item.subtitle} ${item.descriptor}`;
    const tokens = normalizeForNlp(text);
    const weight = recencyWeight(item.publishedAt, now);

    tokens.forEach((token) => {
      unigram.set(token, (unigram.get(token) ?? 0) + 1 + weight);
    });

    for (let i = 0; i < tokens.length - 1; i += 1) {
      const pair = `${tokens[i]} ${tokens[i + 1]}`;
      bigram.set(pair, (bigram.get(pair) ?? 0) + 1 + weight);
    }

    const descriptor = normalizeForNlp(item.descriptor).join(" ").trim();
    if (descriptor) {
      descriptors.set(descriptor, (descriptors.get(descriptor) ?? 0) + 1 + weight);
    }

    const stamp = safeDate(item.publishedAt);
    if (stamp) {
  ...
  return {
    topKeywords: topByValue(unigram, 40),
    topBigrams: topByValue(bigram, 20),
    topDescriptors: topByValue(descriptors, 20),
    articlesCount: items.length,
    dateRange: { min: minDate, max: maxDate },
  };
}

function normalizeForNlp(value: string): string[] {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
    ...
    .filter((token) => token.length >= 3 && !STOPWORDS_ES.has(token));
}

function recencyWeight(publishedAt: string, now: number): number {
  const stamp = safeDate(publishedAt);
  if (!stamp) return 0;
...
function safeDate(input: string): number | null {
  const t = Date.parse(input);
  return Number.isFinite(t) ? t : null;
}

function topByValue(map: Map<string, number>, limit: number): string[] {
  return [...map.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit)
    .map(([k]) => k);
...
export function buildExport(items: ArticlePreview[], format: ExportFormat) {
  if (format === "json") {
    return {
